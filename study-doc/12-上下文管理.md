...
flowchart TD
    ...
    C --> D{estimatedPromptTokens 大于 promptBudgetBeforeReserve?}
    D -- no --> E[fits 🟢 直接发送到 LLM]
    D -- yes --> F{toolResultReducibleChars 大于 0?}
...
```mermaid
flowchart TB
    A(sumarizeInStages 入口) --> B{检查消息量超出 reserveTokens?}
    B -- 否 --> C[summarizeWithFallback 单次摘要]
    B -- 是 --> D[splitMessagesByTokenShare 按 token 份额切片]
    D --> E[Chunk 1]
    D --> F[Chunk 2]
    D --> G[Chunk N]
    E --> H[各自 summarizeWithFallback
      重试 3 次 指数退避 500-5000ms]
    F --> I[各自 summarizeWithFallback
      重试 3 次 指数退避 500-5000ms]
    G --> J[各自 summarizeWithFallback
      重试 3 次 指数退避 500-5000ms]
    H --> K[Merge 合并
      再调用 LLM 合并多段摘要]
    I --> K
