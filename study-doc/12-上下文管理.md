...
> 📊 完整流程图见：`study-doc/diagrams/12-上下文管理流程.html`
>（打开即可查看，支持亮/暗色模式自动切换）
...
│ Layer A: 系统提示组装                                           │
│  ┌──────────────────────────┐  ┌──────────────┐  ┌───────────┐ │
│  │ 稳定区 (Cacheable)       │  │ 动态区       │  │ 缓存控制  │ │
│  │ SOUL/MEMORY/工具列表     │  │ 日志/频道指令│  │ ephemeral │ │
│  └──────────────────────────┘  └──────────────┘  └───────────┘ │
│  │ ← CACHE_BOUNDARY → │     sort → 稳定区在边界之上           │
├──────────────────────────────────────────────────────────────────┤
│ Layer B: Turn 前 → 溢出预检 → 路由决策                          │
│  ① context-engine maintain()                                    │
│  ② overflow precheck (估算 × 1.2)                              │
│  ③ 路由: fits / truncate / compact / compact_then_truncate     │
├──────────────────────────────────────────────────────────────────┤
│ Layer C: 压缩管道                                               │
│  summarizeInStages()                                            │
│    ├── token 不超限 → summarizeWithFallback()                   │
│    └── 超限 → splitMessagesByTokenShare() → 逐 chunk 摘要 → merge│
│  强制保留清单 · 标识符保护 · previousSummary 连续性              │
│  渐进式 fallback: 过滤 → 重试 → partial → 最终 fallback        │
├──────────────────────────────────────────────────────────────────┤
│ Layer D: 图片剪枝 + 微压缩                                      │
│  旧图片 → 文本标记 · 保留最近 3 轮                             │
│  tool result 截断 · thinking 块裁剪 · 图片降级                  │
├──────────────────────────────────────────────────────────────────┤
│ Layer E: LLM 调用 → before-tool-call → turn 完成 → 后台维护     │
└──────────────────────────────────────────────────────────────────┘
```
