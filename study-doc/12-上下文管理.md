...
历史消息
│
├── 长度检查（preemptive compaction）
│     ├── fits → 原样发送
│     ├── truncate → tool result 截断
│     └── compact → summarizeInStages()
│                     ├── 分段摘要（chunks）
│                     ├── 合并摘要（merge）
│                     └── 标识符保护 + previousSummary 连续性
│
├── 图片剪枝（3轮外 → 文本标记）
│
└── 微压缩（in-memory，不写磁盘）
      tool result 截断 + thinking 块剪枝
...
LLM 回复
│
├── 工具调用 → before-tool-call 检查 → 执行 → toolResult
│
└── 文本回复

turn 完成
│
└── context-engine deferred maintenance（后台异步）
      更新 transcript、释放内存
```
